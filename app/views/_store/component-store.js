...
export default class ComponentStore {
			...
			cards: {
				...
				list: {
					...
					'card-v2': {
						...
						html:
							'<link rel="stylesheet" href="./css/card-v2.css"> <div class="container margin-top-lg"> <div class="grid gap-xs"> <div class="col-6@sm col-4@md margin-x-auto"> <div class="card card-v2"> <figure class="card__img"><img src="images/card-v2/images/card_1.jpeg" alt="Image description"/> <figcaption class="card__caption"> <div class="card__caption-title">Card Title</div> <div class="card__caption-description">Label</div> </figcaption> </figure> </div> </div> </div> </div> <script src="./js/card-v2.js"></script>',
					},
					'card-v3': {
						id: generateId(5),
						name: 'card-v3',
					...
					'card-v9': {
						...
						html:
							'<link rel="stylesheet" href="./css/card-v9.css"> <div class="container margin-top-lg"> <div class="grid gap-xs"> <div class="col-6images col-4images margin-x-auto"> <div class="card card-v9"><a class="card__link" href="#" title="Description of the link" aria-label="Description of the link" style="background-image: url(&quot;images/card-v9/images/card_1.jpeg&quot;)"> <div class="card__body"> <div class="card__content"> <p class="card__label">Category</p> <h3 class="card__title">Lorem ipsum dolor sit amet consectetur adipisicing elit. Hic, atque.</h3> </div> <div class="card__button"><span><i>Read more</i></span></div> </div></a></div> </div> </div> </div> <script src="./js/card-v9.js"></script>',
					},
					'card-v10': {
						id: generateId(5),
						name: 'card-v10',
	...
	deletePage(event) {
		this.dispatch({
			pages: this.state.pages.filter(
				(page) => page.id !== event.detail.id
			),
		});
	}

	addComponent(event) {
		const pageIndex = this.state.pages.findIndex(
			(page) => page.id === event.detail.pageId
		);
		const accordionIndex = this.state.accordions.findIndex(
			(accordion) => accordion.id === event.detail.accordionId
		);
		const accordion = this.state.accordions[accordionIndex];
		const componentIndex = accordion.components.findIndex(
			(component) => component.id === event.detail.componentId
		);

		this.state.pages[pageIndex].components.push(
			accordion.components[componentIndex].id
		);
